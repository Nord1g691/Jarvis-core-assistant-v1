...
function installSettingsUI() {
  ...
  function renderCardSettings(){
    draftCards={...getCards()};draftOrder=[...getOrder()];dirty=false;saveButton.disabled=true;saveButton.classList.remove("jarvisDirty");cardList.innerHTML="";
  ...
  function moveCard(index,delta){const target=index+delta;if(target<0||target>=draftOrder.length)return;[draftOrder[index],draftOrder[target]]=[draftOrder[target],draftOrder[index]];const currentCards={...draftCards};renderCardSettings();draftCards=currentCards;markDirty()}
