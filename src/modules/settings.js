...
function installSettingsUI() {
  ...
  function renderCardSettings(){draftCards={...getCards()};draftOrder=[...getOrder()];dirty=false;saveButton.disabled=true;saveButton.classList.remove("jarvisDirty");cardList.innerHTML="";draftOrder.forEach(key=>{const row=document.createElement("div");row.className="jarvisCardRow";row.draggable=true;row.dataset.cardKey=key;row.innerHTML=`<span class="jarvisDrag">☷</span><span class="jarvisCardName">${CARD_LABELS[key]}</span><label class="jarvisSwitch"><input type="checkbox" ${draftCards[key]?"checked":""}><span class="jarvisSlider"></span></label>`;row.querySelector("input").onchange=e=>{draftCards[key]=e.target.checked;markDirty()};row.ondragstart=()=>row.classList.add("dragging");row.ondragend=()=>{row.classList.remove("dragging");draftOrder=[...cardList.querySelectorAll(".jarvisCardRow")].map(el=>el.dataset.cardKey);markDirty()};row.ondragover=e=>{e.preventDefault();const dragging=cardList.querySelector(".dragging");if(dragging&&dragging!==row){const box=row.getBoundingClientRect();cardList.insertBefore(dragging,e.clientY>box.top+box.height/2?row.nextSibling:row)}};cardList.appendChild(row)})}
