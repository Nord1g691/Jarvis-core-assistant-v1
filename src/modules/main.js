...
function setState(state) { setCoreState(state, music); const systemState = $("systemState"); if (systemState) systemState.textContent = connected ? (state === "idle" ? "CONNECTÉ" : state.toUpperCase()) : "HORS LIGNE"; }
function setConnectionVisual(ok) { const state=$("coreState"); const hint=$("coreHint"); const system=$("systemState"); if(ok){state.textContent="CONNECTÉ";state.style.color="#6cff8d";hint.textContent="HOME ASSISTANT • WEBSOCKET";if(system)system.textContent="CONNECTÉ";}else{state.textContent="ERREUR DE CONNEXION";state.style.color="#ff5b6e";hint.textContent="HOME ASSISTANT INDISPONIBLE";if(system)system.textContent="HORS LIGNE";} }
function speak(text) { if (muted || !window.speechSynthesis) return; window.speechSynthesis.cancel(); const u = new SpeechSynthesisUtterance(text); u.lang="fr-FR"; u.volume=.75; u.rate=.95; u.pitch=.85; u.onstart=()=>{setState("responding");$("micStatus").textContent="JARVIS PARLE..."}; u.onend=async()=>{setState("idle");if(!muted&&connected){const r=ensureRecognition();if(r&&!listening)await startRecognition(r)}}; u.onerror=()=>{logError("Erreur synthèse vocale.");setState("idle")}; window.speechSynthesis.speak(u); }
async function testHA() { const token=getToken(); if(!token){connected=false;setConnectionVisual(false);$("coreState").textContent="CONFIGURATION";$("coreState").style.color="#ffb84d";$("coreHint").textContent="TOKEN HOME ASSISTANT REQUIS";$("micStatus").textContent="BLOQUÉ";logError("Token Home Assistant requis.");return false;} connected=await testHomeAssistant(token); if(connected){setConnectionVisual(true);ensureRecognition();if($("micStatus")?.textContent==="BLOQUÉ")$("micStatus").textContent="SERVICE VOCAL À TESTER";$("ci").placeholder="Commande à JARVIS...";}else{setConnectionVisual(false);$("micStatus").textContent="BLOQUÉ";} return connected; }
